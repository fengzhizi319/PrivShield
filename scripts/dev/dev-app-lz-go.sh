#!/usr/bin/env bash
# ============================================================================
# 【开发模式】一键启动 Go 原生引擎 + 调度之眼控制台 (全部 4 上游 + App-LZ BFF + Vite HMR)
# Launch PrivShield App-LZ Console in DEV mode (Go Engine)
#   (Go Engine :8079 + Hub :8082 + Datasource :8083 + Audit :8084 + BFF :8085 + Web :5174)
#
# 与 dev-app-lz.sh 的区别：
#   - dev-app-lz.sh 使用 Python 引擎（$PYTHON -m engine.server）
#   - 本脚本使用 Go 原生引擎（go run ./cmd/privshield-agent）
#
# 用法 / Usage:
#   ./scripts/dev/dev-app-lz-go.sh [--force] [--skip-upstream]
#
# 选项:
...
start_engine() {
    ...
    _wait_for_http "Go Engine" "$ENGINE_HEALTH_URL" 15 && \
        ...
        echo "⚠️  Go Engine 启动超时，请检查 ${LOGS_DIR}/agent_app_lz_go.log"
}
...
# ── 0. 启动上游服务（Go Engine + 3 Go 微服务）──────────────────────
if [[ "$SKIP_UPSTREAM" != "true" ]]; then
    echo ""
    echo "── 检查并启动上游微服务（Go 引擎版）──"
    start_engine
    start_service_hub
    start_datasource_mgr
    start_audit_log
...
echo "=================================================================="
echo " 🚀 启动 PrivShield App-LZ 调度全景控制台 [Go 引擎 + 开发模式 (HMR)]"
echo "=================================================================="
echo "  Go Engine 引擎:  $ENGINE_HEALTH_URL (REST) / 127.0.0.1:50051 (gRPC)"
