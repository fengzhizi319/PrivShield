#!/usr/bin/env bash
# ==============================================================================
# 脚本名称: stop_all_services_go.sh
# 脚本说明: 优雅关闭 PrivShield Go 原生引擎后台拉起的所有服务实例。
#
# 与 stop_all_services.sh 的区别：
#   - stop_all_services.sh 停止 Python 引擎（engine.server / engine.main / uvicorn）
#   - 本脚本停止 Go 原生引擎（privshield-agent / go-build）
#
# 执行步骤总览：
#   1. 读取 .pids/agent.pid 并发送 SIGTERM 终止 Go Agent 主进程
#   2. 读取 .pids/console.pid 并发送 SIGTERM 终止 Console 代理后端进程
#   3. 读取 .pids/*.pid 终止各微服务进程 (service-hub, datasource-mgr, audit-log)
#   4. 通过 pkill 按进程全名进行幂等兜底清理（Go 引擎进程名）
#
# 用法 / Usage:
#   ./scripts/dev/stop_all_services_go.sh
# ==============================================================================
...
echo -e "${BLUE}====================================================${NC}"
echo -e "${BLUE} 正在优雅停止 PrivShield Go 引擎全栈服务集群...${NC}"
echo -e "${BLUE}====================================================${NC}"
...
for dir in "$PIDS_DIR" "$LEGACY_PIDS_DIR"; do
    if [[ -d "$dir" ]]; then
        stop_by_pid "${dir}/agent.pid" "Go Agent 引擎"
...
# ── 步骤 2：按进程名兜底清理 (Go 引擎进程) ─────────────────────────────────
# Go 引擎进程名（替代 Python 的 engine.server / engine.main / uvicorn）
pkill -f "privshield-agent" 2>/dev/null || true
pkill -f "go-build.*privshield" 2>/dev/null || true
pkill -f "go run ./cmd/privshield-agent" 2>/dev/null || true
# BFF 与微服务进程（与 Python 版相同）
...
echo -e "${GREEN}所有 Go 引擎相关服务实例已成功停止！${NC}"
