...
# it via ``from privacy_local_agent.main import service``.
# 再导出 PrivacyService 单例，使测试和外部调用方可通过
# ``from privacy_local_agent.main import service`` 获取。
...
# =============================================================================
# Phase 5: Standalone entry point — ``python -m privacy_local_agent.main``
# Phase 5: 独立入口 — ``python -m privacy_local_agent.main``
# =============================================================================
...
# For production, use ``python -m privacy_local_agent.server`` which binds 0.0.0.0.
# 该代码块仅在直接运行文件时执行（导入时不执行）。
# 解析命令行参数并启动 Uvicorn ASGI 服务器。
# 注意：此入口默认绑定 127.0.0.1（仅本地开发）。
# 生产环境请使用 ``python -m privacy_local_agent.server``（默认绑定 0.0.0.0）。
