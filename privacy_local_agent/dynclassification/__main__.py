...
def main() -> int:
    ...
    validate_parser = subparsers.add_parser(
        "validate",
        help="校验规则配置 YAML 文件合法性",
    )
    validate_parser.add_argument(
        "rules_dir",
        nargs="?",
        default="rules",
        help="规则配置根目录路径（默认: rules）",
    )
...
def _cmd_validate(rules_dir: str) -> int:
    ...
    print(f"正在校验规则目录: {rules_path.resolve()}")
    print("-" * 60)
    ...
    print("-" * 60)
    if result.is_valid:
        print("✅ 校验通过：所有规则配置文件合法")
        return 0
    else:
        print("❌ 校验失败：存在配置错误，请修复后重试")
        return 1
