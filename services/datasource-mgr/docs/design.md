...
### 1.2 核心特性保留
1. **双协议通信**：支持 HTTP REST (`:8083`) 与 gRPC (`:50053`)；
2. **mTLS 双向认证**：支持 TLS 1.3 证书校验与客户端公钥固定（Public Key Pinning）；
...
graph TD
    ...
    subgraph MockDatasourceMgr [Mock Datasource Mgr 微服务 :8083 / :50053]
        HTTPRouter[Gin REST 路由层<br/>/api/v1/* /api/datasources/* :8083]
        GRPCRouter[gRPC Server :50053<br/>mTLS + Key Pinning]
        MiddlewareStack[轻量中间件链<br/>Auth / RequestID / Logger / Recovery / CORS]

        MockDataProvider[内置模拟数据提供者<br/>data_provider.go]
