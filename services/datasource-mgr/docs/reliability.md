...
这里的“无状态”是指服务不会将一次请求的进度、会话或待处理任务写入自身的本地数据库。请求到达任一健康副本后，副本都可独立生成并返回模拟数据；副本崩溃或滚动替换不会遗留需要本服务恢复的 `pending` 或 `running` 任务。它并不表示调用结果已在跨服务链路中持久化：service-hub 在调用前保存任务状态，并负责超时、重试、幂等性与最终状态写入。
...
datasource-mgr 不创建、领取或推进异步任务，因此没有类似 service-hub 的 `pending`、`running`、`completed` 或 `failed` 任务状态，也没有 SQLite `TaskStore`。一次 HTTP 或 gRPC 调用只根据请求参数生成模拟数据并返回响应；服务重启后不存在需要恢复的业务状态。这个边界是有意的架构选择：任务受理、状态持久化、重试次数和退避时间均由调用方 service-hub 管理，datasource-mgr 只提供可横向扩展的数据读取能力。

因此，对 datasource-mgr 而言，“状态已保存”的正确含义不是本服务写入本地数据库，而是上游在调用前已持久化其任务状态，并由上游根据调用结果写入完成或失败状态。datasource-mgr 的响应不应被解释为跨服务事务提交：HTTP/gRPC 响应成功仅表示本次模拟数据生成成功；上游在收到响应后发生崩溃时，仍由其自身的崩溃恢复和重试机制决定是否重新调用。请求处理不写业务数据也意味着无本地磁盘 I/O、无 SQLite 单写入者锁竞争和无写入放大，典型性能成本主要来自参数校验、内存中数据生成、JSON 或 Protobuf 编解码以及网络传输。
