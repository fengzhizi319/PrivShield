...
**脱敏审计日志**（Audit Log）是 PrivShield 体系的核心合规存证中枢，记录所有脱敏操作的完整审计轨迹，提供 9 要素密码学连续哈希链（Hash Chain）、快照样本 SM4-GCM 信封加密、单点/全链完整性校验以及多维合规报告生成能力。
...
```
业务操作发生 → 创建审计记录 → 追溯前序 PrevHash → 计算 9 要素哈希链 → SM4-GCM 快照样本加密 → 原子落盘 → 链式防篡改验真 → 定期合规报告
```
...
2. **快照样本信封加密 (SM4-GCM)**：
   快照中的 `input_sample` 与 `output_sample` 在落盘前经 SM4-GCM 应用层信封加密并带有 `enc:v1:` 前缀，防止日志库拖库泄露明文 PII。
