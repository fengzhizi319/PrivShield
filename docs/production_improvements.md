...
为了让 `PrivShield` 达到工业级可用，我们将针对之前梳理出的遗留缺陷，开展以下三项核心改进设计：
...
### 1.1 现状与问题
当前双协议启动入口 `engine/server.py` 将 FastAPI REST 放在守护线程（`daemon=True`）启动，主线程运行 gRPC 并阻塞。
...
### 3.1 现状与问题
虚假查询模板（`MEDICAL_DUMMY`、`GENERIC_DUMMY`）被硬编码在 `engine/privacy/qol.py` 内部。
不同业务在不同行业（如金融、电商）落地时，需要混入与自己行业高度相关的虚假词典才能起到混淆效果，硬编码导致用户无法自行调整。

### 3.2 改进方案
将虚假词典池纳入隐私配置文件中，并支持请求参数级覆盖：
1. **扩展默认参数**：在 `engine/privacy/profile.py` 中，为 `qol` 原语的默认参数增加 `medical_pool` 与 `generic_pool` 默认列表。
2. **参数透传与覆盖**：在 [qol.py](../engine/privacy/qol.py) 中，使 `obfuscate_query` 接收可选的 `medical_pool` 与 `generic_pool` 参数。若传入，则覆盖硬编码的静态列表。
3. **服务层解析**：在 [service.py](../engine/service.py) 调用 `obfuscate_query` 时，先从 `resolver` 中解析出 profile 中的配置或请求体覆盖的词典列表，最终传给算法层。