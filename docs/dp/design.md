...
- [2. 设计目标](#2-设计目标)
- [3. 算法原理](#3-算法原理)
  - [3.1 差分隐私定义](#31-差分隐私定义)
  - [3.2 敏感度](#32-敏感度)
  - [3.3 Laplace 机制](#33-laplace-机制)
  - [3.4 Gaussian 机制](#34-gaussian-机制)
  - [3.5 mean 的组合实现](#35-mean-的组合实现)
  - [3.6 组合定理](#36-组合定理)
  - [3.7 本地差分隐私（Local DP）](#37-本地差分隐私local-dp)
  - [3.8 Noisify 接口设计](#38-noisify-接口设计)
  - [3.9 Chunked 流式聚合](#39-chunked-流式聚合)
  - [3.10 数据适配器](#310-数据适配器)
  - [3.11 流量监控指标](#311-流量监控指标)
  - [3.12 自适应截断（Adaptive Clipping）](#312-自适应截断adaptive-clipping)
  - [3.13 表格级 DP 聚合编排（dp_aggregate）](#313-表格级-dp-聚合编排dp_aggregate)
  - [3.14 高维向量 DP 加噪（vector_sum / vector_mean）](#314-高维向量-dp-加噪vector_sum-vector_mean)
...
- [5. BudgetAccountant 设计](#5-budgetaccountant-设计)
  ...
  - [5.2 预算消耗规则](#52-预算消耗规则)
  - [5.3 存储后端](#53-存储后端)
  - [5.4 超支处理](#54-超支处理)
  - [5.5 Rényi DP 会计（RDPAccountant）](#55-rényi-dp-会计rdpaccountant)
- [6. 数据集级隐私预算分配](#6-数据集级隐私预算分配)
  - [6.1 确定数据集总预算](#61-确定数据集总预算)
  - [6.2 查询级预算分配](#62-查询级预算分配)
  - [6.3 组合定理](#63-组合定理)
  - [6.4 预算分配示例](#64-预算分配示例)
  - [6.5 设计建议](#65-设计建议)
- [7. 接口定义](#7-接口定义)
  - [7.1 REST 请求示例（标准聚合）](#71-rest-请求示例标准聚合)
  - [7.2 REST 请求示例（Noisify）](#72-rest-请求示例noisify)
  - [7.3 REST 请求示例（Chunked）](#73-rest-请求示例chunked)
  - [7.4 gRPC 字段与高效传输](#74-grpc-字段与高效传输)
- [8. 高级机制与架构演进设计](#8-高级机制与架构演进设计)
  - [8.1 结构化输出与 PyArrow 零拷贝 (`DPResult.to_arrow()`)](#81-结构化输出与-pyarrow-零拷贝-dpresultto_arrow)
  - [8.2 用户级差分隐私 (User-Level DP)](#82-用户级差分隐私-user-level-dp)
  - [8.3 整数格 ℤ 上的离散拉普拉斯机制 (Discrete Laplace)](#83-整数格-ℤ-上的离散拉普拉斯机制-discrete-laplace)
  - [8.4 不可篡改 HMAC 预算审计日志 (`BudgetAuditLogger`)](#84-不可篡改-hmac-预算审计日志-budgetauditlogger)
- [9. 安全与兼容性设计](#9-安全与兼容性设计)
- [10. 测试策略](#10-测试策略)
- [11. 生产级增强](#11-生产级增强)
  - [11.1 RDPAccountant 回调钩子集成](#111-rdpaccountant-回调钩子集成)
  - [11.2 SQLite `threading.local()` 连接复用](#112-sqlite-threadinglocal-连接复用)
  - [11.3 结构化审计日志统一](#113-结构化审计日志统一)
  - [11.4 入参 Guardrails 增强](#114-入参-guardrails-增强)
  - [11.5 IEEE 754 浮点累加容差保护](#115-ieee-754-浮点累加容差保护)
- [12. 设计总结与项目评分](#12-设计总结与项目评分)
  - [12.1 核心设计哲学](#121-核心设计哲学)
  - [12.2 能力矩阵](#122-能力矩阵)
  - [12.3 项目评分](#123-项目评分)
  - [12.4 后续演进路线](#124-后续演进路线)
- [13. 工业化评分 / Industrialization Scorecard](#13-工业化评分-industrialization-scorecard)
  - [13.1 加权评分表](#131-加权评分表)
  - [13.2 结论](#132-结论)
  - [13.3 亮点](#133-亮点)
  - [13.4 改进建议](#134-改进建议)
...
本文档定义 `PrivShield` 差分隐私（DP）模块的算法原理、技术架构与实现细节。DP 模块为 count、sum、mean 等聚合查询提供校准噪声注入能力，并通过 BudgetAccountant 追踪累计隐私预算消耗。
