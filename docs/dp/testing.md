...
- [2. 测试目标](#2-测试目标)
- [3. 单元测试策略](#3-单元测试策略)
  - [3.1 机制正确性测试](#31-机制正确性测试)
  - [3.2 参数校验测试](#32-参数校验测试)
  - [3.3 预算消耗测试](#33-预算消耗测试)
  - [3.4 统计特性测试](#34-统计特性测试)
  - [3.5 均值低频保护与直方图测试](#35-均值低频保护与直方图测试)
  - [3.6 本地差分隐私测试](#36-本地差分隐私测试)
  - [3.7 Noisify 接口测试](#37-noisify-接口测试)
  - [3.8 Chunked 接口测试](#38-chunked-接口测试)
  - [3.9 数据适配器测试](#39-数据适配器测试)
  - [3.10 流量监控指标测试](#310-流量监控指标测试)
  - [3.11 时间窗口预算重置测试](#311-时间窗口预算重置测试)
- [3.12 并发压力测试 (`test_budget_concurrency.py`)](#312-并发压力测试-test_budget_concurrencypy)
- [3.13 KS 统计分布检验 (`test_dp_distributions.py`)](#313-ks-统计分布检验-test_dp_distributionspy)
- [4. 集成测试策略](#4-集成测试策略)
  - [4.1 REST 接口测试](#41-rest-接口测试)
  - [4.2 gRPC 接口测试](#42-grpc-接口测试)
- [5. 测试执行命令](#5-测试执行命令)
- [6. 持续集成建议](#6-持续集成建议)
- [7. 验收检查清单](#7-验收检查清单)
...
本文档定义 `engine/privacy/dp.py` 的测试策略、测试范围与可执行示例。该模块同时包含中心式 DP（`DPApi`）与本地 DP（`LocalDPApi`）两类接口，测试需覆盖算法正确性、参数校验、预算消耗、接口一致性、本地 DP 扰动与统计特性。
