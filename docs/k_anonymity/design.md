...
- [2. 设计目标](#2-设计目标)
- [3. 算法原理](#3-算法原理)
  - [3.1 K-匿名定义](#31-k-匿名定义)
  - [3.2 准标识符与敏感属性](#32-准标识符与敏感属性)
  - [3.3 Mondrian 多维分区算法](#33-mondrian-多维分区算法)
  - [3.4 泛化规则](#34-泛化规则)
  - [3.5 信息损失与深度控制](#35-信息损失与深度控制)
  - [3.6 数据集级 vs 单条记录 K-匿名](#36-数据集级-vs-单条记录-k-匿名)
- [4. 算法流程](#4-算法流程)
- [5. 模块设计](#5-模块设计)
  - [5.1 性能与计算优化 (Pandas 向量化 Mondrian)](#51-性能与计算优化-pandas-向量化-mondrian)
  - [5.2 指标](#52-指标)
- [6. 复杂度分析](#6-复杂度分析)
- [7. 接口定义](#7-接口定义)
  - [7.1 REST](#71-rest)
...
- [8. 工业化增强特性](#8-工业化增强特性)
  - [8.1 结构化日志](#81-结构化日志)
  - [8.2 输入校验](#82-输入校验)
  - [8.3 枚举类型安全](#83-枚举类型安全)
  - [8.4 新增泛化层次函数](#84-新增泛化层次函数)
  - [8.5 批量记录泛化](#85-批量记录泛化)
- [9. 测试策略](#9-测试策略)
...
本文档定义 `PrivShield` 数据集级 K-匿名模块的算法原理、技术架构与实现细节。该模块通过泛化准标识符（QI）降低数据重识别风险，确保发布或共享的数据集中每个等价组至少包含 `k` 条记录。
...
**实现模块**: `engine/privacy/kano_table.py`
...
`engine/privacy/data_adapters.py` 提供 `to_records` / `from_records`，用于在降级场景中将 pandas / SecretFlow DataFrame 与记录列表互转。
...
| 维度 | 权重 | 得分 | 说明 |
|------|------|------|------|
| 功能完整性 | 20% | 8/10 | Mondrian 数据集级 + 单记录启发式双模式；数值/分类 QI；DataFrame/SecretFlow 适配；边界保护完备 |
| 性能 | 15% | 8/10 | Pandas 向量化 Mondrian 直通通道，纯 Python 平滑降级；O(n log²n) 复杂度 |
| 可靠性 | 20% | 7/10 | max_depth 防过度泛化，输入校验快速失败；缺少超时保护与并发安全设计 |
| 安全性 | 15% | 7/10 | 严格 K-匿名保证（数据集级）；无差分隐私噪声叠加，单记录模式无统计保证 |
| 可维护性 | 15% | 8/10 | 双语文档完整，type hints + 枚举，模块拆分清晰（kano.py + kano_table.py） |
| 工程化 | 15% | 7/10 | Counter 指标覆盖三个入口，结构化日志完整；缺少延迟 Histogram 与 Tracing |
| **总分** | **100%** | **7.65** | |
...
- Mondrian 算法文档含完整递归示例，可追溯性极强。
- Pandas 向量化与纯 Python 双通道平滑降级。
- 输入校验覆盖 k<2、空 qi_cols、行数<k 等边界。
- `QIType`/`GeneralizationStrategy` 枚举避免魔法字符串。

### 10.4 改进建议

| 优先级 | 建议 | 影响维度 |
|--------|------|----------|
| P1 | 添加 `privacy_kano_duration_seconds` Histogram 指标 | 工程化 +1 |
| P2 | 为 `k_anonymize_table` 添加超时保护（大数据集递归过深） | 可靠性 +1 |
| P3 | 单记录模式添加显式警告日志（提示无 K-匿名保证） | 安全性 +0.5 |
| P3 | 补充并发安全设计文档（当前为无状态函数，线程安全） | 可靠性 +0.5 |