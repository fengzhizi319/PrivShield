...
- [1. 概述](#1-概述)
- [2. 负载均衡与网络安全核心概念通俗解析 (Core Concepts & Primer)](#2-负载均衡与网络安全核心概念通俗解析-core-concepts--primer)
  - [2.1 流量方向：南北向（North-South）与东西向（East-West）](#21-流量方向南北向north-south与东西向east-west)
  - [2.2 负载均衡层级：L4（四层） vs L7（七层）与 gRPC 痛点](#22-负载均衡层级l4四层-vs-l7七层与-grpc-痛点)
  - [2.3 调度算法：轮询、平滑加权轮询与最小连接数](#23-调度算法轮询平滑加权轮询与最小连接数)
  - [2.4 高可用自愈：主动探针、被动感知、熔断器与幂等重试](#24-高可用自愈主动探针被动感知熔断器与幂等重试)
  - [2.5 网络安全与架构术语：Hop-by-Hop、mTLS、Fail-Closed、Headless Service](#25-网络安全与架构术语hop-by-hopmtlsfail-closedheadless-service)
- [3. 设计目标与核心原则](#3-设计目标与核心原则)
- [4. 系统架构与交互拓扑](#4-系统架构与交互拓扑)
  - [4.1 总体拓扑架构](#41-总体拓扑架构)
  - [4.2 流量代理转发流程](#42-流量代理转发流程)
- [5. 核心模块设计与实现细节](#5-核心模块设计与实现细节)
  - [5.1 BackendNode（后端节点模型）](#51-backendnode后端节点模型)
  - [5.2 CircuitBreaker（节点级熔断器）](#52-circuitbreaker节点级熔断器)
  - [5.3 LoadBalancer（负载均衡调度引擎）](#53-loadbalancer负载均衡调度引擎)
  - [5.4 HTTP 反向代理引擎 (`http_proxy.py`)](#54-http-反向代理引擎-http_proxypy)
  - [5.5 gRPC 泛化代理引擎 (`grpc_proxy.py`)](#55-grpc-泛化代理引擎-grpc_proxypy)
  - [5.6 网关统一启动器与生命周期 (`server.py`)](#56-网关统一启动器与生命周期-serverpy)
- [6. 负载均衡与调度算法实现](#6-负载均衡与调度算法实现)
  - [6.1 轮询（Round-Robin）](#61-轮询round-robin)
  - [6.2 平滑加权轮询（Smooth Weighted Round-Robin, Nginx 算法）](#62-平滑加权轮询smooth-weighted-round-robin-nginx-算法)
  - [6.3 随机与加权随机（Random / Weighted Random）](#63-随机与加权随机random--weighted-random)
  - [6.4 最小连接数（Least Connections）](#64-最小连接数least-connections)
- [7. 高可用、健康检查与自愈机制](#7-高可用健康检查与自愈机制)
  - [7.1 双协议主动健康探针（Active Probing）](#71-双协议主动健康探针active-probing)
  - [7.2 被动故障感知与冷却退避（Passive Health Detection）](#72-被动故障感知与冷却退避passive-health-detection)
  - [7.3 熔断器状态机与半开探测](#73-熔断器状态机与半开探测)
  - [7.4 自适应重试与幂等故障转移规则](#74-自适应重试与幂等故障转移规则)
- [8. 安全防护与访问控制架构](#8-安全防护与访问控制架构)
  - [8.1 双向 TLS / mTLS 体系（南北向终结 + 东西向回源）](#81-双向-tls--mtls-体系南北向终结--东西向回源)
  - [8.2 动态拓扑管理与 SSRF 防护（Fail-Closed 策略）](#82-动态拓扑管理与-ssrf-防护fail-closed-策略)
  - [8.3 内部错误脱敏与信息防泄漏（Error Masking）](#83-内部错误脱敏与信息防泄漏error-masking)
- [9. 协议转换与高级代理特性](#9-协议转换与高级代理特性)
  - [9.1 Hop-by-Hop 头过滤与响应压缩解包防重复](#91-hop-by-hop-头过滤与响应压缩解包防重复)
  - [9.2 gRPC 元数据双向透传与 64MB 消息体支持](#92-grpc-元数据双向透传与-64mb-消息体支持)
  - [9.3 真实客户端 IP 透传（X-Forwarded-For / X-Real-IP）](#93-真实客户端-ip-透传x-forwarded-for--x-real-ip)
  - [9.4 事件循环感知与连接池复用（Event Loop Drift Mitigation）](#94-事件循环感知与连接池复用event-loop-drift-mitigation)
- [10. 分布式共享隐私预算记账](#10-分布式共享隐私预算记账)
