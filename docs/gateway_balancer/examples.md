...
- [1. 概述](#1-概述)
- [2. 命令行启动示例](#2-命令行启动示例)
  - [2.1 使用环境变量启动](#21-使用环境变量启动)
  - [2.2 使用 YAML 配置文件启动](#22-使用-yaml-配置文件启动)
- [3. Python SDK 示例](#3-python-sdk-示例)
  - [3.1 创建负载均衡器并添加后端](#31-创建负载均衡器并添加后端)
  - [3.2 创建 HTTP 网关应用](#32-创建-http-网关应用)
  - [3.3 动态注册与注销节点](#33-动态注册与注销节点)
  - [3.4 切换负载均衡策略](#34-切换负载均衡策略)
- [4. REST API 示例](#4-rest-api-示例)
  - [4.1 通过网关访问后端健康检查](#41-通过网关访问后端健康检查)
  - [4.2 通过网关调用脱敏接口](#42-通过网关调用脱敏接口)
  - [4.3 动态注册节点](#43-动态注册节点)
  - [4.4 动态注销节点](#44-动态注销节点)
- [5. 选择不同负载均衡策略](#5-选择不同负载均衡策略)
  - [5.1 轮询（Round-Robin）](#51-轮询round-robin)
  - [5.2 随机（Random）](#52-随机random)
