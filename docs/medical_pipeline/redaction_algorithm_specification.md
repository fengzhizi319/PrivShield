...
#### 规则 1：死因短语自然重构
将包含特定恶性肿瘤、HIV、罕见病的死因描述重构为自然汉语：
- **匹配正则**：`_REDACT_CAUSE_DEATH_PATTERN` / `_REDACT_SUFFER_DEATH_PATTERN`
- **重构逻辑**：`因'恶性肿瘤'去世` / `死于'胃癌'` $\rightarrow$ `因病去世` / `死于`；若后接年龄括号（如 `死于'胃癌'(62岁)`），自动去除孤立括号重构为 **`死于62岁`**。

#### 规则 2：特异性用药与处置完整句法擦除
绑定前缀（口服/给予/启动...）、剂量用法（`20mg qd`）或后缀（控制症状/抗病毒治疗），整句擦除：
- **例**：`建议尽早启动恩替卡韦抗病毒治疗，注意休息` $\rightarrow$ **`注意休息`**（连同前缀 `建议尽早启动` 整块擦除，避免遗留无宾语动词 `启动，`）。

#### 3. 就诊机构与独立诊断擦除
- `曾就诊于精神卫生中心，诊断为重度精神分裂症。` $\rightarrow$ **`""` (整句消除)**。

#### 4. 特征体征与现象倾向擦除
- 擦除 `外阴/会阴/肛周多发赘生物伴瘙痒`、`醋酸白试验阳性`、`咪喹莫特乳膏外用`、`及保护性约束倾向` 等句法块。

#### 5. 范畴化降级泛化重构
对于适宜泛化的病种，应用 `_CATEGORY_GENERALIZATION_RULES`：
- `家族中有明显消化道肿瘤聚集倾向` $\rightarrow$ **`家族中有明显消化道疾病聚集倾向`**
- `有乙肝家族史` $\rightarrow$ **`有肝脏疾病家族史`**

#### 6. 语法自愈与孤立残渣清理 (`_clean_orphan_syntax`)
依次执行以下自愈逻辑：
1. **空括号与死标点清理**：消除擦除产生的 `()`、`“”` 及重复标点 `，，`。
2. **括号年龄自愈**：`死于(62岁)` $\rightarrow$ `死于62岁`。
3. **孤立动词自愈**：`母亲患(55岁确诊)` $\rightarrow$ `母亲患病(55岁确诊)`。
4. **单字边界保护**：严禁无边界匹配单字 `现`、`示`，**严防将 `发现` 删成 `发`、将 `表现` 删成 `表`**。
5. **无主语/无主病因时间从句绝杀**：若整句擦除后仅剩下 `患者1年前`、`患者3年前无明显诱因`、`反复发作3年` 等无意义时间状语，直接清空为 **`""`**。
...
2. 结合实体类型擦除关联的修饰前缀与后缀。
3. 擦除后统一经过 `_clean_orphan_syntax` 语法自愈。
...
- **拼音/同音/形近变体**（如 `aizibing`、`肺ai`、`霉毒`）不在词库匹配范畴，依赖 Layer-3 本地 LLM 语义识别兜底；当前版本此类变体可能残留。
