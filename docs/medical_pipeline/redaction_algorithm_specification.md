...
## 8. 医保结算数据模式 (`yibao.csv` 18 字段) 治理规约
...
| 5 | 入院日期 | `admission_date` | **L2 (中)** | 日期截断为年月 / 同一患者随机天数统一偏移 (Random Date Offset) | §3.1, §3.3 |
| 6 | 出院日期 | `discharge_date` | **L2 (中)** | 日期截断为年月 / 同一患者随机天数统一偏移 (保留住院天数差) | §3.1, §3.3 |
| 7 | 住院天数 | `length_of_stay` | **L1 (低)** | 原样保留 (数值型衍生属性) | §2.2 |
| 8 | 入院科室 | `admission_dept` | **L1 (低)** | 原样保留 / 离线科研模式下执行列洗牌 (Column Shuffle) | §2.2, §3.3 |
| 9 | 出院科室 | `discharge_dept` | **L1 (低)** | 原样保留 / 离线科研模式下执行列洗牌 (Column Shuffle) | §2.2, §3.3 |
