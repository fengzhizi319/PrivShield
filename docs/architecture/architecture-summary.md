...
> **适用范围**：`PrivShield` 核心算力引擎（`engine-go` / `privacy-go-sdk`）、企业级中台微服务群（`service-hub` / `datasource-mgr` / `audit-log`）、控制台与双 BFF 体系（`bff-go` / `app-lz` / `web`）及全局云原生基础设施。  
...
- **算力面 (PrivShield Core & Primitives)**：Go 1.25+（`engine-go/` + `privacy-go-sdk/`）实现的零内存分配、多核分块并发隐私原语（国密 SM3/SM4、脱敏、差分隐私、K-匿名与 L-多样性、查询混淆）与 3 层动态分类分级漏斗（Rule → Small-NER → External LLM）；
...
├── engine-go/            → Go 核心隐私算力引擎与动态分类分级 (REST :8079 / gRPC :50051)
│   ├── cmd/              → privshield-agent 与 privshield-gateway 主入口
│   └── internal/         → 3-Layer 漏斗、网关反代、DICOM 影像脱敏、安全鉴权、可观测性
├── privacy-go-sdk/       → 纯 Go 零依赖无状态隐私计算数学原语库
│   ├── masking/          → 掩码原语 (国密 SM3/SM4、身份证、手机、银行卡、姓名、邮箱)
│   ├── dp/               → 差分隐私 (单趟融合向量 DP、Laplace/Gaussian、自适应截断)
│   ├── ldp/              → 本地差分隐私 (多核分块随机响应、样本守恒校准)
│   ├── kano/             → K-匿名 (Mondrian KD-tree 空间切分) 与 Distinct L-多样性检验
│   ├── medical/          → 医疗治理流水线 (多核并发分块、ICD-10 规则库)
│   ├── qol/              → 查询混淆 (Fisher-Yates 语义置乱)
│   └── budget/           → 无锁原子预算会计 (无锁 CAS 循环 + 原子回滚)
...
- Agent 统一由 `engine-go/cmd/privshield-agent` 启动，原生同时监听 REST (`:8079`) 与 gRPC (`:50051`)；
- 网关反向代理统一由 `engine-go/cmd/privshield-gateway` 启动，监听 REST (`:8000`) 与 gRPC (`:50000`)。
...
| **国密支持** | SM3 哈希 / SM4-GCM 加密 | GB/T 32918.4 密码哈希与 GB/T 32907 对称加密，快照持久化数据带 `enc:v1:` 前缀加密 |
...
- **P2C-EWMA 负载均衡 (`engine-go/internal/gateway/balancer.go`)**：Power of Two Choices 算法结合指数加权移动平均（EWMA）延迟与在途请求动态打分，消除羊群效应；
- **BufferPool 零分配反向代理 (`engine-go/internal/gateway/http_proxy.go`)**：基于 `sync.Pool` 复用 32KB 数据包切片，实现反向代理数据流转发 0 堆内存分配；
- **gRPC 零反序列化流代理 (`engine-go/internal/gateway/grpc_proxy.go`)**：采用 `rawCodec` 直通模式，避免 Proto 编解码 CPU 损耗。
