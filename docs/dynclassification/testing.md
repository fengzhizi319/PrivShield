...
- [1. 测试策略概述](#1-测试策略概述)
- [2. 测试文件结构](#2-测试文件结构)
- [3. 单元测试代码示例](#3-单元测试代码示例)
  - [3.1 `OperatorRegistry` 算子注册与调用测试](#31-operatorregistry-算子注册与调用测试)
  - [3.2 `ConfigurableRuleEngine` 评估引擎测试](#32-configurableruleengine-评估引擎测试)
  - [3.3 三层漏斗 NER 配置化测试](#33-三层漏斗-ner-配置化测试)
- [4. CI 中 YAML 规则 Schema 自动校验](#4-ci-中-yaml-规则-schema-自动校验)
  - [校验命令](#校验命令)
  - [Schema 校验示例](#schema-校验示例)
- [5. 运行全套单元测试](#5-运行全套单元测试)
