...
为进一步提升自动解析的**代码韧性**，系统已引入以下控制机制：

1. **降低误报率（False Positive Reduction）**：
   * **否定与排除语义识别 (`_is_effective_positive_hit`)**：在匹配关键词时检测上下文中是否带有“不适用于”、“不包括”、“不涉及”、“免责”等否定或排除修饰词，若为排除语义则自动忽略，防止非适用规则的误生成。
   * **非正文章节隔离**：在解析时自动隔离与忽略“参考文献”、“前言起草说明”等辅助章节，避免参考文献提及的敏感词干扰核心规则的提取。
2. **提高正确率/召回率（Precision & Recall Improvement）**：
   * **扩展同义词与多形态表达词库**：引入近义特征词识别（如“身份证”扩展包含“公民身份号码”、“居民身份证”、“身份证件”；“电话”扩展包含“移动电话”、“固定电话”、“联系电话”）。
3. **单元测试与验证**：
   在 `tests/dynclassification/test_standard_doc_generator.py` 中补充了针对否定句过滤（`test_parser_resilience_negation_filter`）与同义词召回（`test_parser_resilience_synonym_trigger`）的自动化测试，确保代码在面临异构输入时具备良好的韧性。
