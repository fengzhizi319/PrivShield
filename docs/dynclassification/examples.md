...
- [1. 分级标准文档一键自动生成 YAML 配置](#1-分级标准文档一键自动生成-yaml-配置)
  - [1.1 CLI 命令行方式一键生成](#11-cli-命令行方式一键生成)
  - [1.2 Python SDK 自动化生成与热重载](#12-python-sdk-自动化生成与热重载)
- [2. Python SDK 基础用例](#2-python-sdk-基础用例)
  - [1.1 加载 Profile 并评估单个字段](#11-加载-profile-并评估单个字段)
  - [1.2 医疗行业场景评估 (DB51/T 2989)](#12-医疗行业场景评估-db51t-2989)
- [2. 自定义匹配算子扩展示例](#2-自定义匹配算子扩展示例)
- [3. REST API 调用示例](#3-rest-api-调用示例)
  - [3.1 HTTP curl 请求：评估字段](#31-http-curl-请求评估字段)
  - [3.2 触发规则配置热加载](#32-触发规则配置热加载)
- [4. Python 影子模式（Shadow Mode）对比测试示例](#4-python-影子模式shadow-mode对比测试示例)
