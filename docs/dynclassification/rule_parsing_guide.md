...
- [1. 规则定义示例](#1-规则定义示例)
- [2. 解析流程（加载阶段）](#2-解析流程加载阶段)
  - [2.1 文件读取与 YAML 反序列化](#21-文件读取与-yaml-反序列化)
  - [2.2 Pydantic 模型校验与结构化](#22-pydantic-模型校验与结构化)
  - [2.3 引擎构建与规则排序](#23-引擎构建与规则排序)
- [3. 执行流程（运行时阶段）](#3-执行流程运行时阶段)
  - [3.1 总体调用链](#31-总体调用链)
...
- [4. 算子注册机制](#4-算子注册机制)
  ...
  - [4.3 ICD-10 算子特殊机制](#43-icd-10-算子特殊机制)
- [5. 规则目录路径配置](#5-规则目录路径配置)
  - [5.1 路径解析逻辑](#51-路径解析逻辑)
  - [5.2 目录结构约定](#52-目录结构约定)
  - [5.3 Docker 环境配置](#53-docker-环境配置)
  - [5.4 配置建议总结](#54-配置建议总结)
- [6. 完整数据流总结](#6-完整数据流总结)
