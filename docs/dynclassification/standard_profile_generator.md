...
- [1. 概述与设计背景](#1-概述与设计背景)
- [2. 算法细节说明](#2-算法细节说明)
  - [2.1 标准标识符 (Standard ID) 与描述抽取](#21-标准标识符-standard-id-与描述抽取)
  - [2.2 敏感度等级矩阵抽取 (`_extract_levels`)](#22-敏感度等级矩阵抽取-_extract_levels)
  - [2.3 数据分类目录树抽取 (`_extract_categories`)](#23-数据分类目录树抽取-_extract_categories)
  - [2.4 词条特征抽取与匹配规则自动构建 (`_generate_rules`)](#24-词条特征抽取与匹配规则自动构建-_generate_rules)
  - [2.5 YAML 序列化与文件生成 (`generate_files`)](#25-yaml-序列化与文件生成-generate_files)
- [3. 算法流程图](#3-算法流程图)
  - [3.1 主处理流水线流程图](#31-主处理流水线流程图)
  - [3.2 规则自动构建判断流程图](#32-规则自动构建判断流程图)
- [4. 使用示例](#4-使用示例)
  - [4.1 CLI 命令行使用](#41-cli-命令行使用)
  - [4.2 Python SDK 使用示例](#42-python-sdk-使用示例)
  - [4.3 输入与输出对照示例](#43-输入与输出对照示例)
- [1. 敏感度等级划分](#1-敏感度等级划分)
- [5. 总结](#5-总结)
- [6. 设计健壮性与多标准解析能力评估](#6-设计健壮性与多标准解析能力评估)
  - [6.1 设计健壮性 (Robustness)](#61-设计健壮性-robustness)
  - [6.2 是否能对不同标准都实现“完全正确解析”？（能力边界分析）](#62-是否能对不同标准都实现完全正确解析能力边界分析)
  - [6.3 代码韧性优化（正确率与误报率控制）](#63-代码韧性优化正确率与误报率控制)
