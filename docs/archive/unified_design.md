...
> **版本**：v14.0.0  
> **状态**：✅ **Implemented Blueprint & Migration Record**  
...
4. ~~**SQLite → PostgreSQL 割接**~~ ✅ — `pkg/store/cmd/migrate/main.go`（`scripts/prod/migrate_sqlite_to_postgres.sh` 包装器）提供原子迁移工具，带 snapshot 密文原样迁移与 9 要素审计日志哈希链完整性校验。
