...
> ✅ **Phase 9 已实现**：Service 层 + REST 路由层全面接入 `pkg/naming`，未知/预留数据源 Fail-Closed。
...
| 目标模块 | 当前状态 | 说明 |
|---|---|---|
| `privacy-go-sdk/` | ✅ Phase 14 已实现 | 7 个包：`masking/`、`dp/`、`ldp/`、`kano/`、`qol/`、`budget/`、`medical/`，含单元测试、基准测试与 **Python 跨语言对齐测试**（masking 13 + QOL 4 + DP 11 + LDP 15 + Budget 5 + KAno 6 = 54 个对齐测试）。LDP 模块新增 `PerturbBinaryBatch`/`PerturbCategoricalBatch`/`EstimateBinaryFrequency`/`EstimateCategoricalHistogram`。DP 模块新增 `VectorSum`。 |
...
| `internal/service/`、`internal/rest/`、`internal/grpcserver/` | ✅ Phase 17 已实现 | Service 编排层（**SSOT 数据源归一化 `pkg/naming` + Fail-Closed** + **LDP 批量/频率估计 + QOL 批量 + 预算重置 + DP 直方图/向量**）、REST 路由（**41 个端点统一错误信封** + **SSOT 别名解析** + 57 个集成测试）、gRPC 服务端（UnknownServiceHandler 模式 + **类型安全 TypedServer 34/34 RPC 全覆盖** + 20 个 gRPC 集成测试）。 |
...
| `cmd/privshield-gateway/` | ✅ Phase 8 已重构 | L7 网关入口，补齐 gRPC 透明流代理集成（`grpcProxy.NewGrpcProxyListener`），HTTP+gRPC 双协议代理 + **Prometheus `/metrics` 端点（4 个 gateway 专属指标）** + **代理层指标联动**。 |
...
> **Phase 5 + Phase 9 实现状态**：以下 3/3 项已全部完成集成。

1. **统一错误信封接入 (`pkg/middleware/envelope.go`)**：
   - 彻底废除 Go 引擎中原有的旧版错误响应，统一使用 `middleware.AbortWithError(c, status, code, msg, detail)`；
   - ✅ **Phase 5 已集成**（Agent 入口）+ **Phase 6 已扩展**（REST 全部 17 端点）+ **Phase 8 已扩展**（Gateway HTTP 代理）。
2. **SSOT 数据源别名收敛 (`pkg/naming/`)**：
   - 将所有涉及医保与康养的路由和参数判断，统一收敛至 `naming.DSYibao` 与 `naming.DSKangyang`；入站值使用 `naming.NormalizeDataSourceID(raw)` 一次性完成归一化，未知或预留数据源 fail-closed；
   - ✅ **Phase 9 已实现**：`service.SanitizeMedicalRecord` / `SanitizeMedicalBatch` 使用 `naming.NormalizeDataSourceID()` 替换硬编码字符串比较，REST 层对未知数据源返回 `400 INVALID_DATASOURCE_ID` 统一信封。10 个 service 测试 + 5 个 REST 测试通过。
...
| 修订项 | v20.0.0 状态 | v21.0.0 实现 |
|---|---|---|
| DP REST 路由 | 3 个端点（noisy_count/noisy_sum/noisy_mean） | 新增 14 个端点：count/sum/mean/histogram/noisy_histogram/chunked_count/chunked_sum/chunked_mean/chunked_histogram/vector_sum/vector_mean/aggregate/adaptive_clip/groupby |
| Mask REST 路由 | 3 个端点（mask/record/batch） | 新增 1 个端点：mask/dataframe |
| Health REST 路由 | 1 个端点（GET /health） | 新增 3 个端点：livez/readyz/readyz/llm |
| Service 层 | 缺少 DP 直方图/向量方法 | 新增 DPHistogram/DPVectorSum/DPVectorMean |
| REST 集成测试 | 39 个测试 | 新增 18 个测试 + 信封测试扩展 15 个新端点，总计 57 个 |
| REST 端点总数 | 23 个 | 41 个 |

**Phase 17 实现清单**：
- [x] `engine-go/internal/service/service.go` — 新增 3 个 Service 方法（DPHistogram/DPVectorSum/DPVectorMean）
- [x] `engine-go/internal/rest/routes.go` — 新增 18 个 REST 路由 + 18 个处理器
- [x] `engine-go/internal/rest/routes_test.go` — 新增 18 个集成测试 + 信封测试扩展 15 个新端点
- [x] 全量测试 — privacy-go-sdk (7 包 ok) + engine-go (6 包 ok)，`-race` 全部通过

### v20.0.0 修订（v19.0.0 → v20.0.0）

本次修订完成 Phase 16 实现：gRPC TypedServer 全量 RPC 对齐：

| 修订项 | v19.0.0 状态 | v20.0.0 实现 |
|---|---|---|
| gRPC TypedServer RPC | 16 个已实现 / 34 个 Proto 方法 | 补齐 18 个缺失 RPC 方法，Proto 34/34 全覆盖 |
| LDP 批量扰动/频率估计 RPC | 未实现 | 新增 PerturbBinaryBatch/PerturbCategoricalBatch/EstimateBinaryFrequency/EstimateCategoricalHistogram |
| QOL 批量 RPC | 未实现 | 新增 ObfuscateQueryBatch |
| DP 直方图/分块/向量 RPC | 未实现 | 新增 DPHistogram/DPNoisyHistogram/DPChunkedCount/DPChunkedSum/DPChunkedMean/DPChunkedHistogram/DPVectorSum/DPVectorMean |
| DP 高级 RPC | 未实现 | 新增 DPAggregate/DPAdaptiveClip/DPGroupBy |
| K-匿名/DataFrame RPC | 仅 KAnonymizeRecord | 新增 KAnonymizeTable/KAnonymizeDataFrame/MaskDataFrame |
| Profile 推荐 RPC | 未实现 | 新增 RecommendParams |
| gRPC 集成测试 | 0 个 | 新增 20 个 TypedServer 集成测试 |

**Phase 16 实现清单**：
- [x] `engine-go/internal/grpcserver/typed_server.go` — 补齐 18 个缺失 RPC 方法（LDP 4 + QOL 1 + DP 10 + KAnonymizeTable/DataFrame/MaskDataFrame 3），Proto 34/34 全覆盖
- [x] `engine-go/internal/grpcserver/typed_server_test.go` — 新增 20 个 gRPC 集成测试（）
- [x] 全量测试 — privacy-go-sdk (7 包 ok) + engine-go (6 包 ok)，`-race` 全部通过

### v19.0.0 修订（v18.0.0 → v19.0.0）

本次修订完成 Phase 15 实现：REST 路由对齐 + Service 层补齐：

| 修订项 | v18.0.0 状态 | v19.0.0 实现 |
|---|---|---|
| LDP REST 路由 | 2 个端点（randomized_response/orr） | 新增 4 个端点：perturb/binary、perturb/categorical、estimate/binary、estimate/categorical |
| QOL REST 路由 | 1 个端点（obfuscate） | 新增 1 个端点：obfuscate/batch |
| Budget REST 路由 | 1 个端点（GET budget） | 新增 1 个端点：POST budget/reset |
| Service 层 | 缺少 LDP 批量/估计方法、QOL 批量、预算重置 | 新增 PerturbBinaryBatch/PerturbCategoricalBatch/EstimateBinaryFrequency/EstimateCategoricalHistogram/ObfuscateQueryBatch/BudgetReset |
| REST 集成测试 | 33 个测试 | 新增 6 个测试（LDP 4 + QOL batch 1 + Budget reset 1），总计 39 个 |
| REST 端点总数 | 17 个 | 23 个 |

**Phase 15 实现清单**：
- [x] `engine-go/internal/service/service.go` — 新增 6 个 Service 方法（PerturbBinaryBatch/PerturbCategoricalBatch/EstimateBinaryFrequency/EstimateCategoricalHistogram/ObfuscateQueryBatch/BudgetReset）
- [x] `engine-go/internal/rest/routes.go` — 新增 6 个 REST 路由 + 6 个处理器
- [x] `engine-go/internal/rest/routes_test.go` — 新增 6 个集成测试 + 信封测试扩展 5 个新端点
- [x] 全量测试 — privacy-go-sdk (7 包 ok) + engine-go (6 包 ok)，`-race` 全部通过

### v18.0.0 修订（v17.0.0 → v18.0.0）

本次修订完成 Phase 14 实现：LDP 批量扰动 + 频率估计 + DP VectorSum + 跨语言对齐测试补全：

| 修订项 | v17.0.0 状态 | v18.0.0 实现 |
|---|---|---|
| LDP 批量扰动 | 仅有单值扰动函数（RandomizedResponse/ORRResponse） | 新增 `PerturbBinaryBatch`/`PerturbCategoricalBatch`，与 Python `perturb_binary_batch`/`perturb_categorical_batch` 对齐 |
| LDP 频率估计 | 仅有计数估计（EstimateTrueCount/EstimateFrequency） | 新增 `EstimateBinaryFrequency`（返回 float 频率）/`EstimateCategoricalHistogram`（返回 map 频率分布），与 Python `estimate_binary_frequency`/`estimate_categorical_histogram` 对齐 |
| DP 向量求和 | 仅有 `VectorMean` | 新增 `VectorSum`，与 Python `dp_vector_sum` 对齐 |
| 跨语言对齐测试 | 37 个对齐测试 | 新增 17 个对齐测试（LDP 15 + DP 2），总计 54 个对齐测试 |

**Phase 14 实现清单**：
- [x] `privacy-go-sdk/ldp/ldp.go` — 新增 `PerturbBinaryBatch`/`PerturbCategoricalBatch`/`EstimateBinaryFrequency`/`EstimateCategoricalHistogram` + 内部辅助函数
- [x] `privacy-go-sdk/ldp/align_python_test.go` — LDP 跨语言对齐测试（15 个测试）：频率估计/直方图估计/批量扰动/统计特性验证
- [x] `privacy-go-sdk/dp/dp.go` — 新增 `VectorSum` 向量差分隐私求和函数
- [x] `privacy-go-sdk/dp/align_python_test.go` — 新增 `VectorSum` 跨语言对齐测试（2 个测试）
- [x] 全量测试 — privacy-go-sdk (7 包 ok) + engine-go (6 包 ok)，`-race` 全部通过
...
本次修订完成 Phase 12 实现：AC 自动机算子 + 正则修复 + DP 扩展 + 跨语言对齐测试补全：

| 修订项 | v15.0.0 状态 | v16.0.0 实现 |
|---|---|---|
| AC 自动机算子 | `OpACAutomaton` 类型已定义但无实现 | 纯 Go Aho-Corasick 自动机（`ac_automaton.go`，245 行）：trie + BFS failure link + O(N+M+Z) 多模式匹配 + `AcAutomatonOperator` 算子集成 |
| `cachedRegex` 正则匹配 | 简化实现（`strings.Contains` 替代正则） | 使用 `regexp.Compile` + `sync.Map` 缓存，支持完整正则语法 |
| DP `NoisyHistogram` | 不存在 | 新增分类直方图带噪计算，每个分类独立 Laplace 噪声（敏感度=1） |
| DP `VectorMean` | 不存在 | 新增向量均值带噪计算：L2 截断 → 均值 → Laplace 向量噪声 |
| DP 跨语言对齐测试 | 不存在 | 新增 `dp/align_python_test.go`（9 个测试）：ClipValue/ClipL2Norm 精确匹配 + NoisyCount/NoisySum/NoisyHistogram/VectorMean 统计无偏性验证 |
| Budget 跨语言对齐测试 | 不存在 | 新增 `budget/align_python_test.go`（5 个测试）：初始预算/扣减拒绝/重置/边界条件/零 delta |
| AC 自动机测试 | 不存在 | 新增 `ac_automaton_test.go`（14 个测试 + 2 个基准）：单模式/多模式/重叠/空输入/医学词库/偏移精度/算子集成/注册表集成 |

**Phase 12 实现清单**：
- [x] `engine-go/internal/dynclassification/ac_automaton.go` — 纯 Go AC 自动机（trie + BFS failure link + 245 行）
- [x] `engine-go/internal/dynclassification/operators.go` — `AcAutomatonOperator` 算子 + `MatchDetail` 详细匹配 + 正则修复（`regexp.Compile`）
- [x] `engine-go/internal/dynclassification/ac_automaton_test.go` — 14 个测试 + 2 个基准（单模式/多模式/重叠/医学词库/偏移精度/算子/注册表）
- [x] `privacy-go-sdk/dp/dp.go` — 新增 `NoisyHistogram` + `VectorMean`
- [x] `privacy-go-sdk/dp/align_python_test.go` — 9 个跨语言对齐测试
- [x] `privacy-go-sdk/dp/dp_test.go` — 修复 `TestNoisySum` 稳定性（多次采样取均值）
- [x] `privacy-go-sdk/budget/align_python_test.go` — 5 个跨语言对齐测试
- [x] 全量测试 — privacy-go-sdk (7 包 ok) + engine-go (6 包 ok)，`-race` 全部通过

### v15.0.0 修订（v14.0.0 → v15.0.0）

本次修订完成 Phase 11 实现：Go/Python 跨语言测试对齐：

| 修订项 | v14.0.0 状态 | v15.0.0 实现 |
|---|---|---|
| `MaskChineseName` | 3 字姓名输出 `张*丰`（1 星） | 与 Python `mask_name` 对齐：3 字→ `张**丰`（2 星），4 字及以上→ `*(n-2)` |
| `MaskEmail` | 保留首 2 字符 + 中间星号 + @域名 | 与 Python `mask_email` 对齐：长用户名→首+`***`+尾+@域名；短用户名(≤2)→首+`***`+@域名；无@回退 `MaskDefault` |
| `MaskAddress` | 保留前 6 字符 + 变长星号；短地址全星化 | 与 Python `mask_address` 对齐：保留前 6 字符 + 固定 `****`；短地址(≤6)原样返回 |
| `HashHMAC` | hex 编码，输出 64 字符 | 与 Python `hash_value` 对齐：base64 编码截取前 16 字符 |
| `MaskDefault` | 不存在 | 新增默认脱敏策略函数，保留前 prefix 后 suffix，中间 `*` 填充 |
| QOL 诱饵词库 | Go 医疗 20 条/通用 15 条（与 Python 不一致） | 与 Python `MEDICAL_DUMMY`/`GENERIC_DUMMY` 完全一致（20+20 条） |
| 跨语言对齐测试 | 不存在 | 新增 `align_python_test.go`（masking 13 个 + QOL 4 个 + 词库排序一致性 1 个） |

**Phase 11 实现清单**：
- [x] `privacy-go-sdk/masking/masking.go` — 修复 `MaskChineseName`/`MaskEmail`/`MaskAddress`/`HashHMAC` + 新增 `MaskDefault`，全面与 Python 对齐
- [x] `privacy-go-sdk/masking/masking_test.go` — 更新现有测试期望值与 Python 对齐
- [x] `privacy-go-sdk/masking/align_python_test.go` — 13 个跨语言对齐测试（MaskPhone/MaskIdCard/MaskChineseName/MaskEmail/MaskAddress/MaskDefault/HashHMAC/记录集成/截断一致性）
- [x] `privacy-go-sdk/qol/qol.go` — 诱饵词库与 Python `MEDICAL_DUMMY`/`GENERIC_DUMMY` 完全对齐（20+20 条）+ 新增 `MedicalDecoyPool()`/`GeneralDecoyPool()` 访问器
- [x] `privacy-go-sdk/qol/align_python_test.go` — 4 个跨语言对齐测试（词库大小/关键条目/注入结构/排序一致性）
- [x] `privacy-go-sdk/medical/pipeline_test.go` — 更新 `TestSanitizeName` 期望值与 Python 对齐
- [x] 全量测试 — privacy-go-sdk (7 包 ok) + engine-go (6 包 ok)，`-race` 全部通过
...
本次修订完成 Phase 9 实现：SSOT 数据源命名接入（`pkg/naming` 集成）：

| 修订项 | v12.0.0 状态 | v13.0.0 实现 |
|---|---|---|
| Service 层数据源路由 | 硬编码 `"yibao"` / `"kangyang"` 字符串比较，未知数据源静默回落默认流水线 | `SanitizeMedicalRecord` / `SanitizeMedicalBatch` 使用 `naming.NormalizeDataSourceID()` 归一化，支持 canonical id / api_code / 别名统一路由，未知或预留数据源 Fail-Closed 返回 error |
| REST 路由层数据源校验 | 仅 `binding:"required"` 非空校验，无数据源合法性检查 | 新增 `INVALID_DATASOURCE_ID` 错误处理，未知/预留数据源返回 `400` 统一信封 |
| Service 方法签名 | `SanitizeMedicalRecord` 返回 `map[string]string` | 返回 `(map[string]string, error)`，支持 Fail-Closed 错误传播 |
| §3.3 实现状态 | 设计目标，未实现 | ✅ Phase 9 已实现 |
| §13.4 pkg/ 接入进度 | 1/3 项完成 | 3/3 项全部完成 |
| Service 层 SSOT 测试 | 不存在 | 新增 `service_test.go`（10 个测试）：canonical DSID / api_code / slug / 中文别名 / 多别名批量 / 未知 Fail-Closed / 预留 Fail-Closed / 空值 Fail-Closed / 批量 SSOT / 批量 Fail-Closed |
| REST 层 SSOT 测试 | 不存在 | 新增 5 个 SSOT 测试：canonical DSID / 中文别名 / 未知 Fail-Closed / 批量未知 Fail-Closed / 预留 Fail-Closed |

**Phase 9 实现清单**：
- [x] `engine-go/internal/service/service.go` — `SanitizeMedicalRecord` / `SanitizeMedicalBatch` 接入 `pkg/naming` + Fail-Closed
- [x] `engine-go/internal/rest/routes.go` — 医疗端点新增 `INVALID_DATASOURCE_ID` 错误处理
- [x] `engine-go/internal/service/service_test.go` — 10 个 SSOT 测试（归一化 + Fail-Closed）
- [x] `engine-go/internal/rest/routes_test.go` — 5 个 SSOT REST 测试（别名解析 + Fail-Closed 信封校验）
- [x] 全量测试 — engine-go (6 包 ok) + privacy-go-sdk (7 包 ok)，`-race` 全部通过

### v12.0.0 修订（v11.0.0 → v12.0.0）

本次修订完成 Phase 8 实现：网关 Prometheus 指标联动 + 统一错误信封 + 集成测试：

| 修订项 | v11.0.0 状态 | v12.0.0 实现 |
|---|---|---|
| HTTP 代理指标联动 | `GatewayMetrics` 已定义但未接入 `http_proxy.go` | `NewHTTPProxyHandler(lb, metrics)` 每次转发实时上报 `SetBackendInFlight`/`SetBackendEWMALatency`/`SetCircuitBreakerState`/`RecordForwarded` |
| gRPC 代理指标联动 | `GatewayMetrics` 未接入 `grpc_proxy.go` | `NewGrpcProxyServer(lb, metrics)` 在 defer 中上报 InFlight/EWMA，流结束后上报 CB 状态 + 转发计数 |
| HTTP 代理错误信封 | `c.JSON(statusCode, gin.H{...})` 旧版格式 | 全部 3 个错误路径迁移至 `middleware.AbortWithError`，输出统一信封（code/message/detail/trace_id/timestamp） |
| `cbStateString` 辅助函数 | 内联 switch 分散在多处 | 提取为统一函数，`http_proxy.go` + `grpc_proxy.go` 共享 |
| Gateway 集成测试 | 仅 gRPC 代理测试 | 新增 `http_proxy_test.go`（11 个测试）：`cbStateString`/熔断器信封/指标联动/nil metrics 安全/健康检查/信封格式校验/middleware 一致性 |
| Gateway main.go | 未传递 metrics 到代理层 | `NewHTTPProxyHandler(lb, gwMetrics)` + `NewGrpcProxyListener(lb, grpcAddr, gwMetrics)` |

**Phase 8 实现清单**：
- [x] `engine-go/internal/gateway/http_proxy.go` — GatewayMetrics 接入 + 统一错误信封 + `cbStateString` 辅助函数
- [x] `engine-go/internal/gateway/grpc_proxy.go` — GatewayMetrics 接入（defer InFlight/EWMA + 流结束 CB/转发计数）
- [x] `engine-go/internal/gateway/http_proxy_test.go` — 11 个集成测试（代理+指标联动+信封格式+熔断器状态机）
- [x] `engine-go/internal/gateway/grpc_proxy_test.go` — 更新签名适配 `metrics` 参数
- [x] `engine-go/cmd/privshield-gateway/main.go` — 传递 `gwMetrics` 到 HTTP/gRPC 代理层
- [x] 全量测试 — engine-go (5 包 ok) + privacy-go-sdk (7 包 ok)，`-race` 全部通过
...
- [x] 网关 Prometheus 指标联动：`http_proxy.go` + `grpc_proxy.go` 每次转发实时上报 InFlight/EWMA/CB 状态到 Prometheus；
...
- [x] Go/Python 跨语言测试对齐：修复 masking 5 个函数 + QOL 词库对齐 + 26 个对齐测试；
...
- [x] LDP 批量扰动补齐：`PerturbBinaryBatch`/`PerturbCategoricalBatch` + `EstimateBinaryFrequency`/`EstimateCategoricalHistogram` + 15 个对齐测试；
- [x] DP `VectorSum` 向量求和函数 + 2 个对齐测试；
