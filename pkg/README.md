...
| [`pkg/middleware`](./middleware/middleware.go) | 统一 Gin 中间件：API Key 鉴权、CORS 跨域、全链路追踪（`TraceMiddleware`）、结构化日志、Panic Recovery、安全响应头、**跨语言统一错误信封（`AbortWithError` / `RespondWithSuccess`）**以及 **DDoS 纵深防护（IP 令牌桶限流 RateLimit、大包防护 MaxBodySize、并发硬顶 MaxConcurrent）** |
...
| [`pkg/tlsutil`](./tlsutil/tlsutil.go) | 共享 TLS 配置工具：TLS 1.3 强制最低版本、mTLS 双向认证、公钥固定（SPKI Pinning），同时支持 gRPC 与 HTTP 服务器；含 mTLS CN 动态白名单（`DynamicWhitelist`，YAML 文件 mtime 轮询热重载 + per-CN scope 授权） |
...
```go
router := gin.New()
router.Use(middleware.TraceMiddleware())  // 全链路追踪（X-Request-ID + X-Trace-ID 双头注入）
