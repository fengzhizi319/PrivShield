...
├── bff-go/               # Go gRPC API 代理网关 / 主力 BFF (API: :8081)
│   ├── cmd/server/       # 网关启动入口
│   ├── internal/         # gRPC 转换、模型映射、文件解析与压测
│   └── docs/             # Go BFF 专属设计与接口文档
├── bff-py/               # Python REST 代理后端 / 备用 BFF (API: :8080)
...
- **Go BFF 文档**：[design](file:///home/charles/code/sfwork/PrivShield/console/bff-go/docs/design.md) · [api](file:///home/charles/code/sfwork/PrivShield/console/bff-go/docs/api.md) · [test](file:///home/charles/code/sfwork/PrivShield/console/bff-go/docs/test.md) · [ops](file:///home/charles/code/sfwork/PrivShield/console/bff-go/docs/ops.md)
...
| 功能模块 | 描述 | BFF-Go (gRPC) | BFF-Py (REST) |
|---|---|:---:|:---:|
| **Masking** | 字段级、整行记录、批量及结构化 DataFrame 敏感信息脱敏 | ✅ | ✅ |
| **DP / 差分隐私** | 噪声统计（Count/Sum/Mean/Histogram）、向量与自适应裁剪 | ✅ | ✅ |
| **K-Anonymity** | Mondrian 算法数据集泛化与单记录启发式 K-匿名 | ✅ | ✅ |
| **LDP / 本地差分隐私** | 二值/类别特征客户端扰动与中心聚合估计 | ✅ | ✅ |
| **动态分类分级** | 三层漏斗（规则 ➔ Small-NER ➔ 本地 LLM 智能裁决） | ✅ | ✅ |
| **查询混淆 (QOL)** | 差分查询与虚假查询注入 | ✅ | ✅ |
| **中台调度流水线** | 数据服务编排调度（ingest→classify→desensitize→audit） | ✅ (`services`) | ✅ (`services`) |
| **资产与敏感特征发现**| 自动化探测数据源敏感字段并绑定安全级别 | ✅ (`services`) | ✅ (`services`) |
| **合规与存证审计** | 不可篡改 SHA-256 审计追踪与合规报告导出 | ✅ (`services`) | ✅ (`services`) |
